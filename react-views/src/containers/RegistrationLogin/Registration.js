...
class Registration extends Component {
    ...
    registrationHandler = async () => {
        ...
        if (this.state.validForm) {
            ...
            try {
                const postRegister = await axios.post('/focusapi/register', fd);
                console.log(postRegister);
                if (postRegister.data.emailTaken) {
                    let updatedState = this.state;
                    updatedState.validationErrors.push('An account with this e-mail address already exists.');
                    updatedState.progressSpinner = false;
